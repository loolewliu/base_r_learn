...
class(d.class)
```
* 用as_tibble()可以将一个数据框转换为tibble类型, `dplyr`包提供了`filter()`、`select()`、`arrange()`、`mutate()` 等函数用来对tibble选取行子集、列子集，排序、修改或定义新变量，
...
2. data.frame 转化为 tibble
    ...
    * 如果需要将行名转换为结果的一列， 模板为:`dt <- as_tibble(da, rownames="id")`, 则原来的行名变成了dt的id列
    * 如果转换成tibble后仍保留原来的行名:`dt <- as_tibble(da, rownames=NA)`
