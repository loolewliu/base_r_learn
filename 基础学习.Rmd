...
* %in%是比较特殊的比较， x %in% y的运算把向量y看成集合， 运算结果是一个逻辑型向量， x向量中的每个值是否属于y向量。如
```{r}
...
```
函数`duplicated()`返回每个元素是否为重复值的结果，用函数unique()可以返回去掉重复值的结果。如：
...
cat(s)
```
> 注意空**字符串**并不能自动认为是缺失值， 字符型的缺失值仍用NA表示。

### 1.3.2 转义字符和原始字符串

* 了在字符串中表示一个双撇号， 可以用**反斜杠**在前面标明， 称为“转义”，如：
```{r}
cat("\"\n")
```
...
原始字符串如果内容中包含了圆括号， 可以将边界的圆括号改为方括号[]或者大括号{}。 如果这样也不能避免歧义， 可以在开始和结尾加上相同个数的减号， 格式为r"--(...)--"， 其中...为实际内容， 减号个数可以根据需要增加。
```{r}
...
```{r}
x <- c(3, 4, 3, 5, 7, 5, 9)
x[which(x > 5)]     # 返回x向量大于5的值
x[which.min(x)]     # 返回x向量中最小值
...
ages[c(1,3)]
``` 
* 用 `unname(x)` 返回去掉了元素名的x的副本，x向量值不变， 用 `names(x) <- NULL` 可以去掉x的元素名，不返回任何。
